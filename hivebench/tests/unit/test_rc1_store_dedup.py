...
bookkeeping: the re-ingest bumps times_saved and refreshes recency, while the
...
def test_first_save_counts_once():
    store = ContextStore()
    cid = store.add_chunk(1, "novel content here")
    assert store.chunks[cid].times_saved == 1
...
def test_distinct_content_still_stores_separately():
    ...
    assert store.chunks[a].times_saved == 1
    assert store.chunks[b].times_saved == 1
...
def test_third_save_keeps_bumping_earliest_copy():
    store = ContextStore()
    ...
    assert store.count() == 1
    assert store.chunks[cid].times_saved == 3
